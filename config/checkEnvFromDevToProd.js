...
module.exports = {
    ...
    modTemplate: function () {
        ...
        for (var key in pre_entry) {
            var cur_path = path.resolve(__dirname, '../dist/' + key + '.html');
            var htmlText = fs.readFileSync(cur_path, 'utf-8');
            var initialIndex = htmlText.indexOf('<title>');
    ...
    modConfig: function (flag) {
        var cur_path = path.resolve(__dirname, '../src/server/logicCheck/config.js');
    ...
    moveTemplateFile: function (sourceFile_path, fileName) {
        var destPath = path.join(__dirname, '../dist/views/' + fileName);
