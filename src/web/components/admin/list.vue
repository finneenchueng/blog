<template>
    <div class="weui-panel weui-panel_access">
        ...
        <modal :show.sync="modalOpt.show" :closeWhenOK.sync="!modalOpt.show" :extendClass="modalOpt.cancelClass"
               @ok="modalConfirm" :title="modalOpt.title">
            ...
            <div class="weui-media-box weui-media-box_text">
                <table class="content-tbl">
                    ...
                    <tr>
                        ...
                        <th>
                            <div class="weui-cell">类别</div>
                        </th>
                        <th>
                            <div class="weui-cell">概要</div>
                        </th>
                        <th>
                            <div class="weui-cell">标签</div>
                        </th>
                        <th>
                            <div class="weui-cell">别名</div>
                        </th>
                        <th width="">
                            <div class="weui-cell">创建时间</div>
                        </th>
                    </tr>
                    </thead>
                    <tbody ref="list_body">
                    <tr v-for="(item,index) in postDataList" :data-index="index" :data-key="item.postId"
                        :data-flag="item.editChked==undefined?0:item.editChked">
                        <th><input class="weui-switch rm-checkbox" type="checkbox"/></th>
                        <th><a href="javascript:;" class="weui-cell weui-cell_link" @click="toDetail">{{item.title}}</a>
                        </th>
                        <!-- <td><div class="weui-cell" style="padding-top:30px;">{{decodeContent(item.content)}}</div></td> -->
                        <td>
                            <div class="weui-cell">{{item.typeName}}</div>
                        </td>
                        <td>
                            <div class="weui-cell">{{item.summary}}</div>
                        </td>
                        <td>
                            <div class="weui-cell">{{item.lblName}}</div>
                        </td>
                        <td>
                            <div class="weui-cell">{{item.alias}}</div>
                        </td>
                        <td @click="toggleEdit">
                            <input type="text" class="weui-input" name="createTime" :value="item.createTime"
                                   @blur="doEdit"/>
